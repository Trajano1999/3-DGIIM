...
{   
    if(!barbero.empty() && sala_espera.get_nwt() >= 1){    //si el barbero esta durmiendo y hay al menos dos clientes
        mtx_cout.lock();
...
void Barberia::siguienteCliente()
{
    if(silla.empty()){
        if(sala_espera.get_nwt() < 2){ 
            mtx_cout.lock();
            cout << "No hay clientes suficientes esperando, el barbero se va a dormir " << endl << flush;
            mtx_cout.unlock();

            barbero.wait();
        }else{
            sala_espera.signal();       // Permite que entren dos clientes a la silla
            sala_espera.signal();
        }
    }
}
...
{
    mtx_cout.lock();
    cout << "\tLos clientes salen de la barberia" << endl << flush;
    mtx_cout.unlock();
