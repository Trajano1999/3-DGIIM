...
void funcion_hebra_consumidora(  )
{
   while(true){
      ...
      int dato;
      sem_wait(ocupadas);
         mtx_primera_ocupada.lock();

            dato = vect[primera_ocupada];
            primera_ocupada = (primera_ocupada+1) % tam_vec;

         mtx_primera_ocupada.unlock();
      sem_signal(libres);
      consumir_dato(dato);
   }
}

// Aquí no se produce una seccion crítica ya que se acceden a 
// distintas zonas de memoria, y además in y out son distintas
...
{
   cout << "----------------------------------------------------------------" << endl
        << "Problema de los productores-consumidores (solución VARIOS FIFO)." << endl
        << "----------------------------------------------------------------" << endl
        << flush ;
