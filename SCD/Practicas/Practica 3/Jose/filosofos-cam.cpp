...
void funcion_camarero() {
   int sentados = 0, max_sentados = num_filosofos-1;
...
void funcion_filosofos(int id) {
   ...
   while (true) {
      ...
      MPI_Ssend(&peticion, 1, MPI_INT, id_cam, etiq_sentar, MPI_COMM_WORLD);
      ...
      MPI_Ssend(&peticion, 1, MPI_INT, id_cam, etiq_levantar, MPI_COMM_WORLD);
...
int main( int argc, char** argv )
{
   ...
   if ( num_procesos == num_procesos_actual )
   {
      ...
      if (id_propio == num_procesos - 1) // el último proceso es el camarero
         funcion_camarero();
