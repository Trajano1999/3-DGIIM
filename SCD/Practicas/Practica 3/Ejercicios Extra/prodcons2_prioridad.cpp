...
void funcion_buffer()
{
   ...
   for(unsigned int i=0; i<num_items*2; ++i){
      ...
      if (etiq_emisor == etiq_consumidor || etiq_emisor == MPI_ANY_TAG){ // si se permite consumir, buscamos el emisor de mayor prioridad, sea productor o consumidor
         while(flag == 0){
            for(id_emisor = num_procesos_esperado-1; id_emisor >= 0; --id_emisor){
               MPI_Iprobe(id_emisor, etiq_emisor, MPI_COMM_WORLD, &flag, &estado);
               if(flag > 0)
                  break;
            }
         }
      ...
      MPI_Recv(&valor, 1, MPI_INT, id_emisor, etiq_emisor, MPI_COMM_WORLD, &estado);
